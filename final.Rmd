...
p1 <- pop_x_acc_by_neighboorhood_dt %>% 
  ggplot(aes(x=Number,y=N.Accidents)) + geom_point(alpha = 0.5) + stat_smooth(method="lm") + 
...
ggplot(df) + geom_point(aes(bus_stops, accidents, color = "Bus")) + 
  geom_point(aes(transports, accidents, color = "Transports")) + 
  geom_smooth(aes(bus_stops, accidents, color = "Bus"), method = "lm") + 
  geom_smooth(aes(transports, accidents, color = "Transports"), method = "lm") +
...
ggplot(df, aes(transports, bus_stops)) + geom_point(alpha = 0.5) +
  geom_smooth(method = "lm") + 
  labs(title = "Correlation between Transport and Bus stops in a Neighborhood", x="Number of transport stops", y="Number of bus stops")
```
...
df[, bus_strata := with(df, cut(bus_stops, breaks = c(0, 30, 60, 90, 137),  
                                labels = c("0-30","31-60","61-90", ">90")))] %>%
  ggplot(aes(transports, accidents)) + 
  geom_point(alpha = 0.5) + 
  geom_smooth(method = "lm") + 
  facet_wrap(~ bus_strata, scales = "free") + 
  labs(x="Number of accidents", y ="Number of transport stops", title="Correlation Accidents by Transports controlled for the Number of Bus Stops")
...
# fig.show="hold", fig.height = 4, out.height="25%", fig.align = "center",

# Create the input data frame.
data_dt_1 <- acc_dt[, c(2,5,6,7,8,9,10,11,12,13)] # starting from the 1st data point to all the end. 
...
data_dt_1$District.Name <- as.numeric(as.factor(data_dt_1$District.Name))
...
# ggplot(data = data.table(confusion_matrix),
#        mapping = aes(x = predict_reg,
#                      y = V2)) +
#   geom_tile(aes(fill = N), colour = "white") +
#   geom_text(aes(label = sprintf("%1.0f", N)), vjust = 1) +
#   scale_fill_gradient(low = "white", high = "yellow", trans="log") +
