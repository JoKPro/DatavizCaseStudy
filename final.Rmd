...
However the correlations seem to be stronger than the population correlation the we showed above (Spearman's rho of 0.70/0.74 vs 0.76/0.83 for Neighborhood/District respectively), so it still seems like a good indicator.
...
One could argue that the number of buses in an area is the true driving factor in a given area instead of other transport stops. The latter serve mostly off-road vehicles like trains, underground or boats, but they are probably connected to bus stops that do directly influence street-traffic. 
